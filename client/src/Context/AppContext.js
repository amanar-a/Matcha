...
export default function AppContext(props) {
    ...
    getListInterest: () => {
      axios.get("Users/listInterest").then((data) => {
        if (data.data instanceof Object) {
          ...
          if (!cache.filterData.list || cache.filterData.list.length === 0) cache.filterData.list = data.data.active;
    ...
    search: (search, type) => {
      if (search && ref.changeNotifications && type === "notifications")
        ref.changeNotifications({
          data: [...cache.notifications.data.filter((item) => (item && item.UserName ? item.UserName.indexOf(search) > -1 : ""))],
  ...
  useEffect(() => {
    ref.reconfigAxios();
    try {
      Axios.post("/Authentication/checkifallow")
        .then((result) => {
          changeIsLogin(result.data === "Allow2" ? "Login" : result.data === "Allow1" ? "Step" : "Not login");
          if (result.data === "Allow1") history.push("/information");
        })
        .catch((error) => {});
      ...
      try {
        ...
        socket.current.on("connect", () => {
          ...
          ref.getMessage = (messageObject) => {
            ...
            cache.friends[user.UserName].IsRead =
              cache.userInfo && cache.userInfo.IdUserOwner !== message.IdUserOwner
                ? cache.friends[user.UserName].IsRead + 1
                : cache.friends[user.UserName].IsRead;
