    ...
    if (obj) {
      ...
      if (user.UserName && message && message.Content.trim() && message.Content.trim().length < 255 && socket.UserName) {
        ...
        const result = await mysql.query(
          "SELECT * FROM Friends WHERE `Match`=1 AND ((IdUserOwner=? AND IdUserReceiver=?) OR (IdUserOwner=? AND IdUserReceiver=?))",
          [socket.IdUserOwner, IdUserOwner, IdUserOwner, socket.IdUserOwner]
        );
...
app.get("/resetpass", async function (request, response) {
  if (request.query.token) {
    let sql = "SELECT * FROM Users WHERE Token=?";
    let result = await mysql.query(sql, [request.query.token]);
    if (result.length === 0) {
      response.send("0");
    } else {
      response.send("1");
...
app.get("/login", async function (request, response) {
  if (request.query.act !== "1" && request.query.act !== "2") {
    let req = "SELECT * FROM `Users` WHERE `IsActive`=?";
    let result = await mysql.query(req, [request.query.act]);
    if (result.length > 0) {
      let req = "UPDATE `Users` SET IsActive = 2 WHERE `IsActive`= ?";
      mysql.query(req, [request.query.act]);
      response.writeHead(307, {
        Location: `http://${request.headers.host.split(":")[0]}:3000?tok=` + result[0].Token,
      });
      response.end();
    } else {
      response.writeHead(307, {
        Location: `http://${request.headers.host.split(":")[0]}:3000`,
      });
      response.end();
    }
  } else {
