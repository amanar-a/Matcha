  ...
  if (result.length > 0) {
    ...
    result.map(async (item, index) => {
      const messages = await locals.query(
        ...
        [req.userInfo.IdUserOwner, item.IdUserOwner, item.IdUserOwner, req.userInfo.IdUserOwner]
      );
...
router.post("/Invite", checkIfHasOneImage, async (req, res) => {
  ...
  if (locals.Validate("Username", UserName)) {
    ...
    if (IdUserOwner && IdUserOwner !== req.userInfo.IdUserOwner && ifNotBlock) {
      const checkFriends = await locals.query("SELECT * FROM Friends WHERE (IdUserOwner=? AND IdUserReceiver=?) OR (IdUserOwner=? AND IdUserReceiver=?)", [
