...
mysql.getFriends = async function (idUserOwner) {
  const result = await mysql.query("SELECT Count(*) AS `numberOfFriends` FROM `Friends` WHERE (idUserOwner=? OR IdUserReceiver=?) AND `Match`=1", [
    idUserOwner,
    idUserOwner,
  ]);
  return result;
...
mysql.checkMatch = async function (idUserOwner, IdUserReceiver) {
  const result = await mysql.query(
    "SELECT Count(*) AS `isMatch` FROM `Friends` WHERE (idUserOwner=? AND IdUserReceiver=?) OR (idUserOwner=? AND IdUserReceiver=? AND `Match`=1) ",
